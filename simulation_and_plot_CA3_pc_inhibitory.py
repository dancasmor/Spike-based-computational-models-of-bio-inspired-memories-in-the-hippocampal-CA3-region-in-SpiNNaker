...
def custom_plots(fullPathFile, plot, save, saveName, savePath):
    ...
    for variable in data["variables"]:
        if variable["type"] == "v" and variable["popNameShort"] == "PCL":
            ...
            wPC_PC = variable
        elif variable["type"] == "v" and variable["popNameShort"] == "INHL":
            vINH = variable
        elif variable["type"] == "spikes" and variable["popNameShort"] == "INHL":
            spikesINH = variable

    # Colores a usar en las representaciones
    """
    colors = []
    n = 50
    for i in range(n):
    ...
    # Representamos V y spikes de cada INH
    vMin = data["neuronParameters"][vINH["popNameShort"]]["v_rest"]
    vMax = data["neuronParameters"][vINH["popNameShort"]]["v_thresh"]
    utils.plot_spike_v_comb_all(vINH["data"], spikesINH["data"], [vMin, vMax], 1, data["simTime"], data["timeStep"],
                                vINH["popName"], False, True, plot, save, saveName + "_v_spike_" + vINH["popNameShort"], savePath)
    
    # Representamos los spikes recibidos y emitidos por cada PC
    utils.plot_spike_pre_post_all(spikesPC["data"], spikesDG["data"], data["simTime"], data["timeStep"],
                                  colors, 0.05, spikesDG["popName"], True, plot, save,
                                  saveName+ "_spike_pre_post" + spikesDG["popNameShort"], savePath)
    """
...
def main(plot, save, savePath, execute, executeSTDPCA3, fullPathFile, saveName):
    ...
    :param executeSTDPCA3: si ejecutar CA3 normal o la versión estática
...
if __name__ == "__main__":
    ...
    execute = True
    # + Si ejecutar CA3 normal o la versión estática
